...
import {int, trimFormatNumber} from '../Util/number'
...
export class RadiantMapWriter {
    ...
    public constructor(from: Entities) {
        this._entities = from
    }
    ...
    private parseEntity(entity: Entity, index: int): string {
        ...
        sb += index
        sb += '\r\n{\r\n'
        for (const [key, value] of entity.map.entries()) {
            sb += '"'
            sb += key
            sb += '" "'
            sb += value
    ...
    private parseBrush(brush: MAPBrush, index: int): string {
        ...
        if (brush.patch) {
            ...
            for (const brushSide of brush.sides) {
