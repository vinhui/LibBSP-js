...
export class EntityConnection {
    ...
    public param: string = ''
    public delay: float = 0
    public fireOnce: int = 0
...
export class Entity extends ILumpObject<Entity> {
    ...
    public override set data(value: Uint8Array) {
    ...
    public get modelNumber(): int {
        try {
            if (this.get('classname') === 'worldspawn') {
                ...
                if (this._map.has('model')) {
                    const st = this.get('model')!
                    if (st[0] === '*') {
                        const ret = Number.parseInt(st.substring(1))
                        if (!isNaN(ret)) {
                            return ret
                        }
                    }
    ...
    public static CreateWithClass(className: string, parent?: ILump): Entity {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Entities {
        ...
        const l = new Entities(Entity, undefined, bsp, lumpInfo)
        l.fromData(data)
        return l
    }
    ...
    public parseString(st: string): void {
        if (!this._map) {
            this._map = new Map()
        }
        this._map.clear()
...
//         st = st.replace(/^[\r\n\t]+|[\r\n\t]+$/g, '');
    ...
    public valueIs(key: string, val: string): boolean {
    ...
    public toggleSpawnFlags(bits: uint): void {
        this.spawnFlags ^= bits
    }

    public setSpawnFlags(bits: uint): void {
        this.spawnFlags |= bits
    }
    ...
    protected ctorCopy(source: Entity, parent: ILump): void {
