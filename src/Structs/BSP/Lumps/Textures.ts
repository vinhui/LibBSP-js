...
export class Textures extends Lump<Texture> {
    public override get length(): int {
        if (MapType.IsSubtypeOf(this.bsp.mapType, MapType.Quake2)
            ...
            for (let i = 0; i < this._backingArray.length; ++i) {
                length += (this._backingArray[i]?.name?.length ?? 0) + 1
            }
            ...
            for (let i = 0; i < this._backingArray.length; ++i) {
                const texture = this._backingArray[i]
                if (texture && texture.mipmapFullOffset > 0) {
                    let mipLength = 0
                    ...
                    if (MapType.IsSubtypeOf(this.bsp.mapType, MapType.GoldSrc)) {
                        let paletteLength = (texture.palette?.length ?? 0) + 2
    ...
    public static CreateFromData(data: Uint8Array, structLength: int, bsp: BSP, lumpInfo: LumpInfo = new LumpInfo()): Textures {
        ...
        const c = new Textures(Texture, undefined, bsp, lumpInfo)
        c.fromData(data, structLength)
        return c
    }

    public override fromData(data: Uint8Array, structLength: int): void {
        const view = new DataView(data.buffer)

        if (MapType.IsSubtypeOf(this.bsp.mapType, MapType.Quake)) {
            const numElements = view.getInt32(0, true)
            structLength = 40
            ...
            for (let i = 0; i < numElements; ++i) {
                let myBytes: Uint8Array = new Uint8Array(0)
    ...
    public getTextureAtOffset(offset: int): string | null | undefined {
        let current = 0
        for (let i = 0; i < this._backingArray.length; i++) {
            if (current < offset) {
                current += (this._backingArray[i]?.name?.length ?? 0) + 1
            } else {
    ...
    public getOffsetOf(name: string): number {
        let offset = 0
        for (let i = 0; i < this._backingArray.length; i++) {
            if (this._backingArray[i]?.name?.localeCompare(name, undefined, {sensitivity: 'base'}) === 0) {
                return offset
            } else {
                offset += (this._backingArray[i]?.name?.length ?? 0) + 1
            }
        }
        return -1
    }

    public override getBytes(_offset: int): Uint8Array {
        if (MapType.IsSubtypeOf(this.bsp.mapType, MapType.Quake2)
            || MapType.IsSubtypeOf(this.bsp.mapType, MapType.Quake3)
            || this.bsp.mapType === MapType.Nightfire) {
            ...
            for (let i = 0; i < this._backingArray.length; ++i) {
                ...
                if (texture.mipmapFullOffset > 0
                    ...
                    if (MapType.IsSubtypeOf(this.bsp.mapType, MapType.GoldSrc)) {
                        ...
                        offset += Math.trunc(texture.dimensions.x * texture.dimensions.y / 64)
                        view.setInt16(offset, texture.palette?.length ?? 0, true)
                        offset += 2
            ...
            for (let i = 0; i < this._backingArray.length; ++i) {
                if (this._backingArray[i]?.name?.length === 0 && this._backingArray[i]?.mipmapFullOffset === 0) {
                    ...
                    const newLumpBytes = new Uint8Array(offset + (textureBytes[i]?.length ?? 0))
