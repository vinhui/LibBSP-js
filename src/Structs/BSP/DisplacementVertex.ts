...
export class DisplacementVertex extends ILumpObject<DisplacementVertex> {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<DisplacementVertex> {
        ...
        const l = new Lump<DisplacementVertex>(DisplacementVertex, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: DisplacementVertex, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(DisplacementVertex.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
