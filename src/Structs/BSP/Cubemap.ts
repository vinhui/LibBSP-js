...
export class Cubemap extends ILumpObject<Cubemap> {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Cubemap> {
        ...
        const l = new Lump<Cubemap>(Cubemap, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Cubemap, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Cubemap.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
