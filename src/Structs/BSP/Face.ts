...
export class Face extends ILumpObject<Face> {
    public readonly refObjectMap: { [key: string]: { count: () => number, index: () => number } } = {
        'edges': {
            count: () => this.numEdgeIndices,
            index: () => this.firstEdgeIndexIndex,
        },
        'vertices': {
    ...
    public get planeSide(): boolean {
        if (MapType.IsSubtypeOf(this.mapType, MapType.Quake)
            ...
            return (this.data[34] ?? 0) > 0
        } else if (this.mapType === MapType.Source18) {
            return (this.data[18] ?? 0) > 0
        } else if (MapType.IsSubtypeOf(this.mapType, MapType.Source)) {
            return (this.data[2] ?? 0) > 0
        }
    ...
    public get isOnNode(): boolean {
        if (this.mapType === MapType.Source17) {
            return (this.data[35] ?? 0) > 0
        } else if (this.mapType === MapType.Source18) {
            return (this.data[19] ?? 0) > 0
        } else if (this.mapType === MapType.Vindictus) {
            return (this.data[5] ?? 0) > 0
        } else if (MapType.IsSubtypeOf(this.mapType, MapType.Source)) {
    ...
    public get edgeIndices(): int[] {
        ...
        for (let i = 0; i < this.numEdgeIndices; ++i) {
            arr.push(Number(this._parent?.bsp.faceEdges!.get(this.firstEdgeIndexIndex + i)))
        }
        return arr
    }
    ...
    public get vertices(): Vertex[] {
        ...
        for (let i = 0; i < this.numVertices; ++i) {
            arr.push(this._parent!.bsp.vertices!.get(this.firstVertexIndex + i)!)
        }
        return arr
    }
    ...
    public get material(): Texture | undefined {
    ...
    public get textureInfo(): TextureInfo | undefined {
    ...
    public get lightmapTextureInfo(): TextureInfo | undefined {
    ...
    public get originalFace(): Face | undefined {
    ...
    public get indices(): int[] {
        ...
        for (let i = 0; i < this.numIndices; ++i) {
            arr.push(Number(this._parent?.bsp.indices!.get(this.firstIndexIndex + i)))
        }
        return arr
    }
    ...
    public get averageLightColors(): Color[] {
        if (this.mapType === MapType.Source17) {
            const colors = []
            for (let i = 0; i < 8; ++i) {
                colors[i] = ColorExtensions.FromArgb(this.data[(i * 4) + 3] ?? 0, this.data[(i * 4)] ?? 0, this.data[(i * 4) + 1] ?? 0, this.data[(i * 4) + 2] ?? 0)
            }
            return colors
        }
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Face> {
        ...
        const l = new Lump<Face>(Face, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Face, parent: ILump): void {
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Face.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
