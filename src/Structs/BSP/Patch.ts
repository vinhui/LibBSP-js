...
export class Patch extends ILumpObject<Patch> {
    public readonly refObjectMap: { [key: string]: { count: () => number, index: () => number } } = {
        'patchVertices': {
            ...
            index: () => this.firstVertex,
        },
        'patchIndices': {
            count: () => this.numVertexIndices,
            index: () => this.firstVertexIndex,
        },
    }

    public get shader(): Texture | undefined {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Patch> {
        ...
        const l = new Lump<Patch>(Patch, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Patch, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Patch.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
