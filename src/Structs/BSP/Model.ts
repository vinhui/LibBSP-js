...
export class Model extends ILumpObject<Model> {
    public readonly refObjectMap: { [key: string]: { count: () => number, index: () => number } } = {
        'nodes': {
            count: () => this.headNodeIndex,
            index: () => {
                throw new Error('not implemented')
            },
        },
        'leaves': {
            count: () => this.numLeaves,
            index: () => this.firstLeafIndex,
        },
        'brushes': {
            count: () => this.numBrushes,
            index: () => this.firstBrushIndex,
        },
        'faces': {
            count: () => this.numFaces,
            index: () => this.firstFaceIndex,
        },
        'patchIndices': {
    ...
    public get headNode(): Node | undefined {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Model> {
        ...
        const l = new Lump<Model>(Model, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Model, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Model.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
