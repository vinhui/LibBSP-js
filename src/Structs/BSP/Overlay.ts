...
import {int, uint} from '../../Util/number'
...
export class Overlay extends ILumpObject<Overlay> {
    ...
    public set faceIndices(value: int[]) {
        ...
        for (let i = 0; i < value.length && i < Overlay.NumOverlayFaces; i++) {
            const view = new DataView(this.data.buffer)
            view.setInt32(offset, value[i] ?? 0)
            offset += 4
        }
    }
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Overlay> {
        ...
        const l = new Lump<Overlay>(Overlay, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Overlay, parent: ILump): void {
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Overlay.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
