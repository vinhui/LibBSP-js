...
export class Edge extends ILumpObject<Edge> {
    public get firstVertex(): Vertex | undefined {
    ...
    public get secondVertex(): Vertex | undefined {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Edge> {
        ...
        const l = new Lump<Edge>(Edge, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Edge, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Edge.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
