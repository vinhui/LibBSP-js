...
export class Node extends ILumpObject<Node> {
    ...
    public get child1(): ILumpObject<unknown> | undefined {
    ...
    public get child2(): ILumpObject<unknown> | undefined {
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Node> {
        ...
        const l = new Lump<Node>(Node, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Node, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Node.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
