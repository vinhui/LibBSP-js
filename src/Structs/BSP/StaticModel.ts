...
export class StaticModel extends ILumpObject<StaticModel> {
    ...
    public set name(value: string | null) {
        if (MapType.IsSubtypeOf(this.mapType, MapType.MOHAA)) {
            ...
            for (let i = 0; i < 127; i++) {
                this.data[i] = value?.charCodeAt(i) ?? 0
            }
        }
    }
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<StaticModel> {
        ...
        const l = new Lump<StaticModel>(StaticModel, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: StaticModel, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(StaticModel.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
