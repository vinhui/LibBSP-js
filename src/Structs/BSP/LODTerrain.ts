...
import {byte, float, int, sbyte, short} from '../../Util/number'
import {BSP, LumpInfo, MapType} from './BSP'
import {Texture} from './Texture'
import {generate2dTypedArray} from '../../Util/array'
...
export class LODTerrain extends ILumpObject<LODTerrain> {
    ...
    public get lightmapCoordinates(): Uint8Array | null {
        if (MapType.IsSubtypeOf(this.mapType, MapType.MOHAA)) {
    ...
    public set lightmapCoordinates(value: Uint8Array | null) {
    ...
    public set uvs(value: float[] | null) {
        ...
        if (MapType.IsSubtypeOf(this.mapType, MapType.MOHAA)) {
            ...
            for (let i = 0; i < value.length; ++i) {
                const view = new DataView(this.data.buffer)
                view.setFloat32(offset + (i * 4), value[i] ?? 0, true)
            }
        }
    }
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<LODTerrain> {
        ...
        const l = new Lump<LODTerrain>(LODTerrain, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: LODTerrain, parent: ILump): void {
        this._parent = parent
        ...
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(LODTerrain.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
