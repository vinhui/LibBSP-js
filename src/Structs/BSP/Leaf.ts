...
export class Leaf extends ILumpObject<Leaf> {
    public readonly refObjectMap: { [key: string]: { count: () => number, index: () => number } } = {
        'leafBrushes': {
            count: () => this.numMarkBrushIndices,
            index: () => this.firstMarkBrushIndex,
        },
        'leafFaces': {
            count: () => this.numMarkFaceIndices,
            index: () => this.firstMarkFaceIndex,
        },
        'leafStaticModels': {
            count: () => this.numLeafStaticModelIndices,
            index: () => this.firstLeafStaticModelIndex,
        },
        'patchIndices': {
            count: () => this.numPatchIndices,
            index: () => this.firstPatchIndicesIndex,
        },
    }
    ...
    public get markBrushes(): int[] {
        ...
        for (let i = 0; i < this.numMarkBrushIndices; ++i) {
            arr.push(Number(this._parent?.bsp?.leafBrushes?.get(this.firstMarkBrushIndex + i)))
        }
        return arr
    }
    ...
    public get markFaces(): int[] {
        ...
        for (let i = 0; i < this.numMarkFaceIndices; ++i
            ...
            arr.push(Number(this._parent?.bsp?.leafFaces?.get(this.firstMarkFaceIndex + i)))
        }
        return arr
    }
    ...
    public get leafStaticModels(): int[] {
        ...
        for (let i = 0; i < this.numLeafStaticModelIndices; ++i
            ...
            arr.push(Number(this._parent?.bsp?.leafStaticModels?.get(this.firstLeafStaticModelIndex + i)))
        }
        return arr
    }
    ...
    public get patchIndices(): int[] {
        ...
        for (let i = 0; i < this.numPatchIndices; ++i
            ...
            arr.push(Number(this._parent?.bsp?.leafPatches?.get(this.firstPatchIndicesIndex + i)))
        }
        return arr
    }
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Lump<Leaf> {
        ...
        const l = new Lump<Leaf>(Leaf, undefined, bsp, lumpInfo)
    ...
    protected ctorCopy(source: Leaf, parent: ILump): void {
        this._parent = parent
        if (parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Leaf.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
