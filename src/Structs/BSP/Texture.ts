...
export class Texture extends ILumpObject<Texture> {
    ...
    public set name(val: string | undefined | null) {
        let bytes = new TextEncoder().encode(val ?? undefined)
    ...
    public set mask(val: string | undefined | null) {
        if (MapType.IsSubtypeOf(this.mapType, MapType.MOHAA)) {
            for (let i = 0; i < 64; i++) {
                this.data[i + 76] = 0
            }
    ...
    public get textureInfo(): TextureInfo {
        ...
        return new TextureInfo(new LumpObjDataCtor(new Uint8Array(), undefined))
    }
    ...
    public static LumpFactory(data: Uint8Array, bsp: BSP, lumpInfo: LumpInfo): Textures {
        if (!data) {
            throw new Error('ArgumentNullException')
        }
    ...
    public static GetStructLength(_type: MapType, _version: int): number {
        return -1
    }
    ...
    protected ctorCopy(source: Texture, parent: ILump): void {
        if (!parent?.bsp) {
            ...
            if (source.parent?.bsp) {
                this.data = new Uint8Array(Texture.GetStructLength(source.parent?.bsp.mapType ?? MapType.Undefined, source.parent?.lumpInfo.version ?? 0))
            } else {
