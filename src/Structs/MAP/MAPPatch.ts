import {byte, parseFloatUS} from '../../Util/number'
        ...
        switch (lines[0]) {
            ...
            case 'patchDef2': {
                ...
                for (let i = 0; i < this.dims.x; ++i) {
                    ...
                    for (let j = 0; j < this.dims.y; ++j) {
                        const point = new Vector3(parseFloatUS(line[2 + (j * 7)]!), parseFloatUS(line[3 + (j * 7)]!), parseFloatUS(line[4 + (j * 7)]!))
                        const uv = new Vector2(parseFloatUS(line[5 + (j * 7)]!), parseFloatUS(line[6 + (j * 7)]!))
            ...
            case 'patchTerrainDef3': {
                ...
                for (let i = 0; i < this.dims.x; ++i) {
                    line = lines[i + 5]!.split(' ').filter(x => x)
                    for (let j = 0; j < this.dims.y; ++j) {
                        const point = new Vector3(parseFloatUS(line[2 + (j * 12)]!), parseFloatUS(line[3 + (j * 12)]!), parseFloatUS(line[4 + (j * 12)]!))
                        const uv = new Vector2(parseFloatUS(line[5 + (j * 12)]!), parseFloatUS(line[6 + (j * 12)]!))
